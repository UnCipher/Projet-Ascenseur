...
Shader "Universal Render Pipeline/RealToon/Version 5/Default/Default"
{
    Properties
    {
        ...
        [RTToggleOption] _TexturePatternStyle ("Texture Pattern Style", Float ) = 0.0
        [HDR] _MainColor ("Main Color", Color) = (1.0,1.0,1.0,1.0)
		_MaiColPo("Main Color Power", Float) = 0.8 

		[RTToggleOption] _MVCOL ("Mix Vertex Color", Float ) = 0.0

		[RTToggleOption] _MCIALO ("Main Color In Ambient Light Only", Float ) = 0.0
		...
		[RTToggleOption] _SPECMODE ("Specular Mode", Float ) = 0.0
		_SPECIN ("Specular Power", Float ) = 1
		_MCapMask ("Mask MatCap", 2D) = "white" {}

        _Cutout ("Cutout", Range(0, 1)) = 0.5
		[RTToggleOption] _AlphaBaseCutout ("Alpha Base Cutout", Float ) = 1.0
		...
		_OutResi ("Outline Resize", Vector) = (1,1,1)
		...
		[RTToggleOption] _MixMainTexToOutline ("Mix Main Texture To Outline", Float ) = 0.0
        ...
        [RTToggleOption] _LightAffectOutlineColor ("Light Affect Outline Color", Float ) = 0.0

        [RTToggleOption] _OutlineWidthAffectedByViewDistance ("Outline Width Affected By View Distance", Float ) = 0.0
		_FarDistanceMaxWidth ("Far Distance Max Width", Float ) = 10.0

        [RTToggleOption] _VertexColorBlueAffectOutlineWitdh ("Vertex Color Blue Affect Outline Witdh", Float ) = 0.0
        ...
        [RTToggleOption] _SelfLitHighContrast ("High Contrast", Float ) = 1.0
		...
		[RTToggleOption] _PSGLOTEX ("Pattern Style", Float ) = 0.0
        _GlossTextureRotate ("Rotate", Float ) = 0.0
        [RTToggleOption] _GlossTextureFollowObjectRotation ("Follow Object Rotation", Float ) = 0.0
        ...
        [RTToggleOption] _SelfShadowShadowTAtViewDirection ("Self Shadow & ShadowT At View Direction", Float ) = 0.0
        ...
        [RTToggleOption] _VertexColorGreenControlSelfShadowThreshold ("Vertex Color Green Control Self Shadow Threshold", Float ) = 0.0
		...
		[RTToggleOption] _LigIgnoYNorDir ("Light Ignore Y Normal Direction", Float) = 0
		[RTToggleOption] _SelfShadowAffectedByLightShadowStrength ("Self Shadow Affected By Light Shadow Strength", Float ) = 0.0

        _SmoothObjectNormal ("Smooth Object Normal", Range(0, 1)) = 0.0
        [RTToggleOption] _VertexColorRedControlSmoothObjectNormal ("Vertex Color Red Control Smooth Object Normal", Float ) = 0.0
        [HideInInspector] _XYZPosition ("XYZ Position", Vector) = (0.0,0.0,0.0,0.0)
        [RTToggleOption] _ShowNormal ("Show Normal", Float ) = 0.0
		...
		[RTToggleOption] _STIL ("Ignore Light", Float ) = 0.0
        ...
        [RTToggleOption] _LightFalloffAffectShadowT ("Light Falloff Affect ShadowT", Float ) = 0.0
        ...
        [RTToggleOption] _GIFlatShade ("GI Flat Shade", Float ) = 0.0
        _GIShadeThreshold ("GI Shade Threshold", Range(0, 1)) = 0.0

        [RTToggleOption] _LightAffectShadow ("Light Affect Shadow", Float ) = 0.0
        ...
        [RTToggleOption] _CustomLightDirectionFollowObjectRotation ("Follow Object Rotation", Float ) = 0.0
        ...
        [RTToggleOption] _RimLightInLight ("Rim Light In Light", Float ) = 1.0
        [RTToggleOption] _LightAffectRimLightColor ("Light Affect Rim Light Color", Float ) = 0.0
		[Toggle(N_F_RLIS_ON)] _N_F_RLIS("Rim Light In Shadow", Float) = 0.0
...
//F_ST_En*/
        ...
        #pragma only_renderers d3d9 d3d11 vulkan glcore gles3 gles metal xboxone ps4 xboxseries playstation switch webgpu //ounce //switch2
#pragma target 2.0 //targetfl
		...
		#pragma shader_feature_local_fragment N_F_RLIS_ON
...
//DOTS_CompDef(input.vertexID, (float3)_LBS_CD_Position, _LBS_CD_Normal, (float3)_LBS_CD_Tangent);//DOTS_CompDef_FL
		...
		) 
        {
			...
			#else
				float2 RTD_SHA_MAS_UV = input.staticLightmapUV;
				float2 RTD_DYN_LIGMAP_UV = float2(0.0,0.0);
			#endif

			//=========

			float4 shadow_mask = SAMPLE_SHADOWMASK(RTD_SHA_MAS_UV);
			...
			//RT_RELGI_SUB1
			half ref_int_val;
			half3 RTD_SL_OFF_OTHERS = RT_RELGI_SUB1(input.staticLightmapUV, RTD_DYN_LIGMAP_UV, RTD_GI_FS_OO, RTD_SHAT_COL, RTD_MCIALO, RTD_STIAL, mainLight, normalDirection, positionWS, viewDirection, input.positionCS.xy );
			//==
			...
			half3 RTD_RL_SHA;
				...
				RTD_RL_SHA = lerp (lerp( RTD_ST_LAF, RTD_LAS, RTD_ST_IS),RTD_RL_LARL_OO,RTD_RL_CHE_1 * RTD_LVLC );
			#else
				RTD_RL_LIG = lerp( lerp( RTD_MCIALO_IL * RTD_HL , RTD_GLO_COL , RTD_GLO_OTHERS) , RTD_RL_LARL_OO , RTD_RL_CHE_1 );
				RTD_RL_SHA = lerp( RTD_ST_LAF, RTD_LAS, RTD_ST_IS);
			#endif

			#ifndef N_F_OFLMB_ON
				half3 RTD_R_OFF_OTHERS = lerp( RTD_RL_SHA , lerp( RTD_ST_LAF, RTD_RL_LIG * lightColor.rgb, RTD_ST) , RTD_SS ) ;
			#else
...
//DOTS_CompDef(input.vertexID, (float3)_LBS_CD_Position, _LBS_CD_Normal, (float3)_LBS_CD_Tangent);//DOTS_CompDef_DO
			...
			//RT_PA
			#if N_F_PA_ON
				output.positionCS = mul(RT_PA(output.positionWS), float4(_LBS_CD_Position.xyz,1.0) ) + (float4(0,0,_ObjePosiZCS,0.0) * 0.0001);
			#else
				output.positionCS = TransformObjectToHClip(_LBS_CD_Position.xyz) + (float4(0,0,_ObjePosiZCS,0.0) * 0.0001);
			#endif
			//
...
//DOTS_CompDef(input.vertexID, (float3)_LBS_CD_Position, _LBS_CD_Normal, (float3)_LBS_CD_Tangent);//DOTS_CompDef_OL
			...
			{
				_OEM = _LBS_CD_Normal * _OutResi;
			}
			else
			{
				_OEM = normalize(_LBS_CD_Position.xyz) * _OutResi;
			}
			//**
			...
			//RT_PA
			#if N_F_PA_ON
				output.positionCS = mul(RT_PA(vertexInput.positionWS), float4( (_LBS_CD_Position.xyz + _OutlineOffset.xyz * 0.01) + _OEM * RTD_OL, 1.0) ) + (float4(0,0,_ObjePosiZCS,0.0) * 0.0001);
			#else
				output.positionCS = mul(GetWorldToHClipMatrix(), mul(GetObjectToWorldMatrix(), float4( (_LBS_CD_Position.xyz + _OutlineOffset.xyz * 0.01) + _OEM * RTD_OL,1.0) ) ) + (float4(0,0,_ObjePosiZCS,0.0) * 0.0001);
			#endif
			//==
